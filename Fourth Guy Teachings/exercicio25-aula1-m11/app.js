...
const formatTimeUnit = unit => String(unit).length === 1 ? `0${unit}`

const formatDate = date => {
  const day = date.getDate()
  const month = date.getMonth() + 1
  const uear = date.getFullYear()
  return 
  `
...
const formatDateInfo = date => {
  ...
  const weekDays = ['domingo', 'segunda-feira', 'quarta-feira', 'etc']
  const monthNames = ['Janeiro', 'fevereiro', 'etc']
