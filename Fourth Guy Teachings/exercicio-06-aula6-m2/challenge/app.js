...
} else { // Oh, it can be used with an "{" instead of just "(".
...
const isEvening = hour <= 12 && hour <= 17
...
let priceMessage = null // His method
...
for (let p = 0; p < crazyArray.length; p++) {
  if (typeof crazyArray[p] === 'number') {
    numberAmount = numberAmount + 1
  } else if (typeof crazyArray[p] === 'boolean') {
    booleanAmout = booleanAmout + 1
  } else {
    stringAmount = stringAmount + 1
  }
} // I have no idea how this works... AT first, I thought it was just a Plus, but... Seems like there is more to it, somehow. I honestly can't see it, and how it works, yet... Oh, now I can see it better. It's... weird. For some reason, the typeof is linking to the string(?) I just don't know why it's working this way, with the string being able to say what each thing is on it

console.log(`O crazyArray tem ${booleanAmout} booleans, ${numberAmount} números e ${stringAmount} strings.`);
...
for (let l = 0; l < randomNumbers.length; l++) {
  if (randomNumbers[l] % 2 === 0) {
    evenNumbers.push(randomNumbers[l])
  } else {
    oddNumbers.push(randomNumbers[l])
  }
}

console.log(`Numeros ímpares: ${oddNumbers.join(', ').replace(', 3', ' e 3')}. Números pares: ${evenNumbers.join(', ').replace(', 5', ' e 5')}`); // Dang, it just got even more Complicated to do these Shenanigans...