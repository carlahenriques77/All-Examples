...
booksBox.addBooks = booksQuantity => {
  ...
  if (booksBox.booksIn + booksQuantity > booksBox.spaces) {
    const availiableSpaces = books.spaces - booksBox.booksIn
    const firPluralOrSingular = availiableSpaces === 1 ? 'cabe' : 'cabem'
    const pluralOrSingular = availiableSpaces === 1 ? 'livro' : 'livros'
    return `Só cabem mais ${books.spaces - booksBox.booksIn} ${pluralOrSingular}`
  }
...
// Well, ignore the last one. Too much complicated stuff to do something way too simple. I think we can find another method to solve it, one that uses less than 20 Lines, at that