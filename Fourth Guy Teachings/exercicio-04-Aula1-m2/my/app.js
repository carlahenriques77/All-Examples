...
const my3FavoriteTVShows = ["Filme_01", "Serie_02", "Show_03"]
const sentence = `Minhas 3 séries favoritas são: ${my3FavoriteTVShows.join(', ')}.`.replace(', Sh', ' e Sh') // Buddy. Trying to do it alone sometimes feels pretty... Unfair as a "Challenge". Anyway, seems like you can Link an Attribute here just like that. 
...
console.log(my3FavoriteTVShows.length === 3) // Apparently, "===" is ALSO called an "Boolean"??? Pretti weird, right?
...
console.log(`A string que a "typeSentence" armazena tem mais de ${numberToCheck} caracteres? ${String(typeSentence.length > numberToCheck).replace('t', 'T')}.`) // More "useless" stuff. This time, he just found another useless way to make this useless thing about the true first letter to go UpperCase... I wonder if these things are really necessary...? Oh. He added an "Strig" beforehand... No wonder my one wasn't connecting.
