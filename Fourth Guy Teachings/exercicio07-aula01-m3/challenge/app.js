...
if (!animals.includes('leão')) {
  console.log('What a weird way to do it')
} else {
  console.log('Existe um leão... em algum lugar do mundo.')
}
/*
  03
...
for (let z = 0; z < sentence.length; z++) {
  if (sentence[z] === 'certeza') {
    continue
  }
  newSentence += `${sentence[z]} `
}
...
for (let x = 0; x < randomValues.length; x++) {
  if (stringsAmout === 4) {
    break
  }

  if (typeof randomValues[x] === 'string') {
    stringsAmout++
    firstFourString.push(randomValues[x])
  }

  if (typeof randomValues[x] === 'boolean') {
    booleansAmount++
  }
...
console.log(`3 informações sobre o array randomValues:
    ...
    - O array foi iterado por ${totalIterations} vezes.`) // The "fourString" part was pretty interesting, as it will always place an "e" before the last item.
/*
  06
