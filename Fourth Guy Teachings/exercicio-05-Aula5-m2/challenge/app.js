...
for(let l = 1; l <= 10; l++) {
  const loopPhrase = `Esta é a ${l}ª vez que esta frase é exibida no console.` // Note: It have's to be inside the Loop for it to Work... Otherwise, it won't take what the Written thing is about. Maybe it can work outside if it is the "while", though...
  console.log(loopPhrase);
}

for(let g = 1; g <= 10; g++) {
  console.log(`Esta é a ${g}ª vez que esta frase é exibida no console.`);
}
...
for(v = 0; v < cities.length; v++) {
  upperCaseCities.push(cities[v].toUpperCase())
...
for (let s = 0; s < names.length; s++) {
  HTMLTemplate = HTMLTemplate + `<li>${names[s]}</li>`
}
console.log(HTMLTemplate);

let HTMLTemplate01 = ''
for (let j = 0; j < names.length; j++) {
  HTMLTemplate01 += `<li>${names[j]}</li>`
}
console.log(HTMLTemplate01);
...
const arrays = [
  ...
  [8, 32, 16],
  [2, 8, 4]
] // An Array can be used inside another Array.

let arraysSum = 0
for (let x = 0; x < arrays.length; x++) {
  for (let c = 0; c < arrays[x].length; c++) {
    arraysSum += arrays[x][c] // An Loop can be used Inside another Loop. Complicated "equation" ahead... P.S: The Result was 218.
  }
}
