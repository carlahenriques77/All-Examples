...
person.introduction = () => {
  const correctGender = person.gender === 'Feminino' ? 'a' : 'o'
  const agePluralOrSingular = person.age === 1 ? 'ano' : 'anos'
  const meterPluralOrSingular = person.walkedMeteres === 1 ? 'metro' : 'metros'

  return `Oi. Eu sou o ${person.name}, tenho ${person.age} anos, ${person.height} metros de altura, 
  peso ${person.weight} quilos e, só hoje, eu já caminhei ${person.walkedMeters} 
  metros.`
}
